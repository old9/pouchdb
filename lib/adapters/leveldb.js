...
Index.prototype.get = utils.toPromise(function (opts, callback) {
  ...
  if (typeof opts === 'string') {
    return this.byKey.createReadStream({
      start: opts,
      limit: 1
    }).on('data', function (data) {
      if (data.value.key === opts) {
    ...
    if (streamOpts.reverse) {
      if ('startkey' in opts) {
        streamOpts.end = opts.startkey + self.sep + self.sep;
      }
      if ('endkey' in opts) {
        streamOpts.start = opts.endkey;
      }
    } else {
      if ('startkey' in opts) {
        streamOpts.start = opts.startkey;
      }
      if ('endkey' in opts) {
        streamOpts.end = opts.endkey + self.sep + self.sep;
      }
    }
    skip = opts.skip;
    if ('limit' in opts) {
