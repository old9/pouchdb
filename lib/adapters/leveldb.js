...
Index.prototype.get = utils.toPromise(function (opts, callback) {
  ...
  var readStreamOpts = {valueEncoding: 'json'};
  if (typeof from === 'string') {
    readStreamOpts.start = from;
  }
  if (typeof to === 'string') {
    readStreamOpts.end = to;
  }
  var stream = this.byKey.createReadStream(readStreamOpts);
  ...
  stream.on('end', function () {
    if (typeof opts === 'string') {
      ...
      return callback(new Error('key not found: ' + opts));
    }
    ...
    output = typeof limit === 'number' ? output.slice(skip, limit + skip) :
      skip > 0 ? output.slice(skip) : output;
...
Index.prototype.count = utils.toPromise(function (callback) {
  ...
  if (this._offset === false) {
    self.byKey.createReadStream().on('data', function () {
      total++;
    }).on('end', function () {
      this._offset = true;
      self._count = total;
